...
DO_TEXT = False # we do not use text
DO_PUSH = True # we only use notification
...
class MessagingAgent(Agent):
    ...
    def __init__(self):
        ...
        if DO_TEXT: # We need to get twili tokens and other things for sending messages, we do not work on it in this project
            ...
            self.me_from = os.getenv('TWILIO_FROM', 'your-phone-number-if-not-using-env') # the number that twilio gives you
            self.me_to = os.getenv('MY_PHONE_NUMBER', 'your-phone-number-if-not-using-env') # the number that message is sent to 
    ...
    def push(self, text):
        ...
        conn.request("POST", "/1/messages.json",
          urllib.parse.urlencode({
            ...
            "message": text,
            "sound": "cashregister" # this is otional 
